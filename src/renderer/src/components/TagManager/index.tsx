...
import { Plus, Edit, Trash2, Tag as TagIcon, X } from '../../utils/icons';
...
export function TagManager({ bookId, onClose }: TagManagerProps) {
  ...
  const handleDeleteTag = async (tag: Tag) => {
    const confirmed = await modal.confirm({
      ...
      content: (
        <div>
          <p className="mb-2">确定要删除标签 "{tag.name}" 吗？</p>
          <p className="text-sm text-gray-500">删除后，该标签将从所有书籍中移除。</p>
        </div>
      ),
    });
  ...
  const isTagAddedToBook = (tagId: string) => {
      ...
      {showCreateForm ? (
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-sm font-medium text-gray-900 dark:text-white">创建新标签</h3>
            <button
              onClick={() => {
                setShowCreateForm(false);
                setNewTagName('');
                ...
                {PRESET_COLORS.map((color) => (
                  <button
                    ...
                    className={`h-8 w-8 rounded-full transition-transform hover:scale-110 ${
                      newTagColor === color ? 'ring-2 ring-offset-2 ring-gray-400 dark:ring-offset-gray-800' : ''
                    }`}
            ...
            {allTags.map((tag) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  {bookId && isTagAddedToBook(tag.id) && (
                ...
                <div className="flex flex-wrap gap-2">
                  {PRESET_COLORS.map((color) => (
                    <button
                      ...
                      className={`h-8 w-8 rounded-full transition-transform hover:scale-110 ${
                        editingTag.color === color ? 'ring-2 ring-offset-2 ring-gray-400 dark:ring-offset-gray-800' : ''
                      }`}
                      style={{ backgroundColor: color }}
                      title={color}
                    />
